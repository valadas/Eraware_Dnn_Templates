...
import { IDnnPersonaBarMenuSettings } from "./types/IDnnPersonaBarMenuSettings";
...
/** Initializes the module.
  ...
  */
export function init(wrapper: JQuery<HTMLElement>, util: IDnnPersonaBarUtilities, _params: IDnnPersonaBarMenuSettings, callback: () => void): void {
    // jQuery? Where we are going, we don't need jQuery!
    const wrappingElement = wrapper.get(0) as HTMLElement;
    const module = wrappingElement.querySelector("my-app-root") as (Element & Components.MyAppRoot) | null;
    if (module) {
...
export function load(_util: IDnnPersonaBarUtilities, _params: IDnnPersonaBarMenuSettings, callback: () => void): void {
    // Your logic here
    if (typeof callback === "function"){
        callback();
    }
...
export function leave(_params: IDnnPersonaBarMenuSettings, callback: () => void): void {
    // Your logic here
    if (typeof callback === "function"){
        callback();
    }
